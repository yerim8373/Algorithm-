...
public class Main {
        ...
        while(!q.isEmpty()) {
            ...
            for(Node next : A[now.loc]) {
                if(next.dist + dist[now.loc] < dist[next.loc]) {
                    ...
                    q.add(new Node(next.loc, dist[next.loc])); //next.dist 해서 틀림.
                    //★시작부터★ next.loc 까지의 거리는 next.dist이 아닌 dist[next.loc] 으로 갱신되었음.
                }
            }
        }
    }
    ...
    static class Node implements Comparable<Node>{
        ...
        public int compareTo(Node e) { //우선순위 큐를 위해 구현
